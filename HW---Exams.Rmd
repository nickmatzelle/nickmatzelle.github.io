...
imf_growth <- imf_growth %>% 
  mutate(gy = round(gy, digits = 4))
p <- ggplot(imf_growth, aes(x= country, y= gy, fill= country))+
geom_col()+ geom_text(aes(label=as.character(gy), hjust = -0.4))+ coord_flip()
fig_1 <- p + transition_states(quarter, transition_length = 2, state_length =2) +
...
htmlwidgets::saveWidget(fig_climate, file = "test.html")
